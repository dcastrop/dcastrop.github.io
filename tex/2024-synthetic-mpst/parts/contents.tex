...
\begin{frame}{Properties of Synthetic MPST}
  ...
  \Put(40,170){%
    ...
    \begin{infobox}
      ...
      One of the above lemmas is problematic (we are not sure if it is false...)! It should be obvious which one ... But why?
    \end{infobox}
    \end{minipage}
    \end{onlyenv}
  }
...
\begin{frame}{Why is the Previous Lemma Problematic?}
  \begin{center}
...
\sum\left\{
  ...
  \vspace{.5cm}
  \only<3->{Recursion state $\pvar{X}$: $\globalt{4}$}

  \vspace{.5cm}
  \only<5>{We finished, but we never checked whether the other branch is well-typed!}
\end{frame}
