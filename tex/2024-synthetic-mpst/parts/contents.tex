...
T-SKIP.}
  \only<4-8>{Two cases: $\globalt{1} \to \globalt{2}$, and $\globalt{1} \to \globalt{4}$}
  \only<5,6>{

    \vspace{.1cm}
    -- At $\globalt{4}$, we have that $\annlbl \Rq \Rr {\dlbl{\mathsf{END}}()}$

  \vspace{.1cm}
    -- We transition to $\globalt{5}$, where the process is ended, and $\Rr$ can no longer take any action in $\globalt{G}$. }
  \only<7,8>{

    \vspace{.1cm}
    -- At $\globalt{2}$, we have that $\annlbl \Rq \Rr {\dlbl{\mathsf{REQ}}(\sbool)}$
...
\begin{frame}{Properties of Synthetic MPST}
  ...
  \begin{displaymath}
    \begin{array}{@{}l@{}}
      \bullet\; \text{If }\soft \Sm G {\sf Ps}\text{ and }\Sm \sstepsto \Sm'\text{, then there exists }\globalt{G'}\text{ and }\alpha\text{ such that }\\
      \quad \greduce G \alpha {G'}\text{ and }\soft {\Sm'} {G'} {\sf Ps} \\[.4cm]
      \bullet\; \text{If }\soft \Sm G {\sf Ps}\text{ and }\globalt{G}\text{ is not ended, then there exists }\Sm'\text{ such that }\Sm \sstepsto \Sm'.
    \end{array}
  \end{displaymath}
  \Put(40,170){%
    \begin{onlyenv}<2-3>
    ...
    \begin{infobox}
      ...
      One of the above lemmas is false! It should be obvious which one ... But why?
    \end{infobox}
    \end{minipage}
    \end{onlyenv}
  }
