...
\Put(40,200){
  \begin{onlyenv}<3>
    \begin{minipage}{.75\columnwidth}
      \begin{sticky}
        \includegraphics[width=\columnwidth]{figures/deconfined-session-types.pdf}
      \end{sticky}
    \end{minipage}
  \end{onlyenv}
}
...
\begin{frame}{A Few Attempts at Simplifying the Theory}
  ...
  \Put(40,210){%
    \begin{onlyenv}<2->
    \begin{minipage}{.8`  6\columnwidth}
    \begin{sticky}
...
\begin{frame}{A Potential Solution: Unfolding the Process}
  \begin{center}
    \begin{minipage}{.34\columnwidth}
    \begin{bluebox}
      \vspace{-.3cm}
...
\end{tikzpicture}
  ...
  \begin{displaymath}
    \tikzmarkin<1-2>{a}(0.2,-0.8)(-0.2,0.9)
    \sum\left\{
    \begin{array}{@{}l@{}}
    \tikzmarkin<3>{b}(0.1,-0.5)(-0.1,0.6)
    \precbr \Rq {\dlbl{\mathsf{REQ}}} {x} {%
    \mathsf{process}(\pvar{x})\proc{.}\;
    \tikzmarkin<4>{c}(0.1,-0.5)(-0.1,0.6)
    \precur X {%
    \tikzmarkin<5>{d}(0.1,-0.5)(-0.1,0.6)
    \sum\left\{
       \begin{array}{@{}l@{}}
    \tikzmarkin<8>{g}
         \precbr \Rq {\dlbl{\mathsf{REQ}}} {x} {%
  ...
  \vspace{.5cm}
  \only<5->{Recursion state $\pvar{X}$: $\globalt{1}$}
