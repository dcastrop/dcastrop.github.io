...
\begin{frame}[fragile]
  \frametitle{Example I: Folds as Hylomorphisms}
  \centering
...
\begin{frame}[fragile]
  \frametitle{Example II: Nonstructural Recursion}
  \centering
...
split :: [Int] -> TreeF Int [Int]?\tikzmark{qscoalg1}?
split [] = Leaf
split (h : t) = Node l h r
  where
    (l, r) = partition (\x -> x < h) t

merge :: TreeF Int ([Int] -> [Int]) ->?\tikzmark{qsalg1}?
         [Int] -> [Int]
merge Leaf = \acc -> acc
...
qsort = flip f []
  where 
    ...
    \begin{column}{.47\textwidth}\scriptsize
      \begin{tikzcd}
        \tikzmark{qscoalg2}\mhask{TreeF Int [Int]} \arrow[r, dotted] 
        & \mhask{TreeF Int ([Int] -> [Int])} \arrow[d, "\mhask{merge}"] 
        \\
        \mhask{[Int]} \arrow[u, "\mhask{split}"] \arrow[r, dotted] 
